...
@section("title")
    Arsip Bagian &mdash; {{ config("app.name") }}
@endsection
...
@section("content")
    <div class="container-fluid">
        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <div>
                <h1 class="page-title fw-semibold fs-18 mb-0">Arsip Manajemen Bagian</h1>
                <nav>
                    <ol class="breadcrumb mb-0 mt-1">
                        <li class="breadcrumb-item"><a href="{{ route("settings.dashboard") }}">Pengaturan Aplikasi</a></li>
                        <li class="breadcrumb-item"><a href="{{ route("settings.sections.index") }}">Manajemen Bagian</a></li>
                        <li aria-current="page" class="breadcrumb-item active">Arsip</li>
                    </ol>
                </nav>
            </div>
            <div class="ms-md-1 ms-0 mt-2 mt-md-0">
                <a class="btn btn-secondary btn-wave" href="{{ route("settings.sections.index") }}">
            ...
            <div class="col-xl-12">
                <div class="card custom-card">
                    <div class="card-header d-flex flex-wrap justify-content-between align-items-center">
                        ...
                        <form action="{{ route("settings.sections.archives.index") }}" method="GET">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover text-nowrap table-bordered">
                                ...
                                <tbody>
                                    @forelse ($trashedSections as $trashedSection)
                                        <tr>
                                            <td class="text-center">{{ $loop->iteration }}</td>
                                            <td class="text-center">{{ $trashedSection->name }}</td>
                                            <td class="text-center">{{ $trashedSection->code }}</td>
                                            <td class="text-center">{{ $trashedSection->deleted_at ? $trashedSection->deleted_at->format("l, d-m-Y, H:i:s") : "-" }}</td>
                                            <td class="text-center">
                                                <div class="btn-group">
                                                    <form action="{{ route("settings.sections.archives.restore", $trashedSection) }}" class="d-inline" method="POST">
                                                        @csrf
                                                        @method("PUT")
                                                    ...
                                                    <form action="{{ route("settings.sections.archives.force.delete", $trashedSection) }}" id="force-delete-form-{{ $trashedSection->id }}" method="POST">
                                                        @csrf
                                                        @method("DELETE")
                                                        <button class="btn btn-sm btn-outline-danger" onclick="showConfirmationDialog({ formId: 'force-delete-form-{{ $trashedSection->id }}' })" type="button">
